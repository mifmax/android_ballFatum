package com.Maximv.balldefato;

import java.util.Random;

import com.Maximv.balldefato.R;
import com.Maximv.constants.GlobalConstants;
...
public class MainActivity extends Activity implements SensorEventListener {
        ...
        private void showMessage(String message) {
                ...
                animation.setStartOffset(GlobalConstants.START_OFFSET);
                triangle.setVisibility(TextView.VISIBLE);
                animation.setDuration(GlobalConstants.FADE_DURATION);
                triangle.startAnimation(animation);

                vibrator.vibrate(Integer.parseInt(preferences.getString(
                                getString(R.string.vibrate_time_id), GlobalConstants.VIBRATE_TIME)));
        }
        ...
        private boolean isShakeEnough(float x, float y, float z) {
                ...
                if (force > Float.parseFloat(preferences.getString(
                                getString(R.string.threshold_id), GlobalConstants.THRESHOLD))) {
                        shakeCount++;
                        if (shakeCount > Integer.parseInt(preferences.getString(
                                        getString(R.string.shake_count_id), GlobalConstants.SHAKE_COUNT))) {
                                shakeCount = 0;
                                lastX = 0;
                                lastY = 0;
                                lastZ = 0;
